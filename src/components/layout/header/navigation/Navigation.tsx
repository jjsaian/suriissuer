...
import suri from 'assets/images/icons/logo.png';
  ...
  return (
    ...
    >
      <Container>
        <NavbarBrand>
          <Link to={routes.ROOT}>
            <img src={suri} className='logo-name' alt='logo-name'/>
          </Link>
        </NavbarBrand>
        ...
        <Navbar.Collapse className={showNavbar ? 'show' : ''}>
          <Nav className="justify-content-end navbar-nav">
            ...
            {!appState.isAuthenticated && (
              <>
                <Nav.Item>
                  <Nav.Link as={NavLink} activeClassName='is-active' to={routes.APPLICANT_LOGIN}>Apply for VC</Nav.Link>
                </Nav.Item>
                <Nav.Item>
